...
public class ProjectTableModel extends AbstractTableModel {
  ...
  public Object getValueAt(int row, int column) {
    ...
    switch (column) {
      ...
      case ProjectTableModel.COLUMN_TIMEOVERALL:
        o = Integer.valueOf(prj.getSecondsOverall());
        break;
      case ProjectTableModel.COLUMN_TIMETODAY:
        o = Integer.valueOf(prj.getSecondsToday());
        break;
  ...
  public Class<?> getColumnClass(int column) {
    switch (column) {
      ...
      default:
        return String.class;
        // return getValueAt(0, column).getClass();   // WARNING: sorter would
        // throw exception!
    }
  }
    ...
    switch (column) {
      ...
      case ProjectTableModel.COLUMN_CREATED:
        JTimeSchedApp.getLogger()
            .info(
                String.format(
                    "Manually set create date for project '%s' from %s to %s",
                    prj.getTitle(),
  ...
  public void removeProject(int row) {
    ...
    JTimeSchedApp.getLogger()
        .info(
            String.format(
                "Removed project '%s' (time overall: %s, time today: %s)",
                p.getTitle(),
