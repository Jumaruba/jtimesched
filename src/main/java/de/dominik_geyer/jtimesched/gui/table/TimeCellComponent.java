...
public class TimeCellComponent extends JLabel {
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (this.quota > 0) {
      Color color =
          (this.time < this.quota)
              ? TimeCellComponent.BAR_COLOR
              : TimeCellComponent.BAR_COLOR_OVERDUE;
      int width =
          (this.time < this.quota) ? this.getWidth() * this.time / this.quota : this.getWidth();
      ...
      if (width > TimeCellComponent.BAR_THRESHOLD) {
        ...
        g.fillRect(
            0,
            this.getHeight() - TimeCellComponent.HASQUOTA_HEIGHT,
            TimeCellComponent.HASQUOTA_HEIGHT,
            TimeCellComponent.HASQUOTA_HEIGHT);
      }
    }
  }
